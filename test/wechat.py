...
# GitHub
GITHUB_TOKEN = "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"
...
def upload_qiniu(filename, path="blog/moments/"):
    #构建鉴权对象
        ...
        if request.method == "GET":
            ...
            if msg_type == "text":
                ...
                with open(filename, 'wb') as f:
                    f.write(r.content)
                # 上传
                qiniu_picurl = upload_qiniu(filename)
                # 将图片链接改成markdown中的表达形式
                body = "![{}]({})".format(filename, qiniu_picurl)
                # 发布issue
                title = time.strftime('%Y年%m月%d日 %H:%M:%S',time.localtime(time.time()))
                message = create_github_issue(title,body)
                # 告知用户issue是否成功发布
